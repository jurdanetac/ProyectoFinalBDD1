...
public class territorio extends javax.swing.JFrame {
  ...
  private void initComponents() {
    ...
    getContentPane().add(modificar, new org.netbeans.lib.awtextra.AbsoluteConstraints(210, 340, -1, -1));
  ...
  private void modificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_modificarActionPerformed
    ...
    String[] camposDeTextoContenido = new String[camposDeTexto.length + 2];
    int index = 1;
    ...
    camposDeTextoContenido[0] = territorio_id;

    String territorio = String.format("(%s, '%s', '%s', '%s', %s, %s, '%s', %s)", (Object[]) camposDeTextoContenido);
    ...
    if (!relieves.isEmpty()) {
      for (String rlv : relieves) {
        utils.actualizarBase.insertar("territorio_tiene_relieve", String.format("(%s, '%s')", territorio_id, rlv.trim()));
      }
    }

    if (!cuerposDeAgua.isEmpty()) {
      for (String c : cuerposDeAgua) {
        utils.actualizarBase.insertar("territorio_tiene_cuerpo_de_agua", String.format("(%s, '%s')", territorio_id, c.trim()));
      }
    }
    ...
    if (!seresVivos.isEmpty()) {
      for (String str : seresVivos) {
        ...
        utils.actualizarBase.insertar("territorio_tiene_ser_vivo", String.format("(%s, '%s', '%s')", territorio_id, aux[aux.length - 2], aux[aux.length - 1]));
      }
    }
    ...
    if (!idiomas.isEmpty()) {
      ...
      if (porcentajes.length == 0) {
        ...
        for (int i = 0; i < idiomas.size(); i++) {
          ...
          utils.actualizarBase.insertar("idioma_es_utilizado_en_territorio", String.format("('%s', %s, %s, %s)", idioma, territorio_id, porcentajes[i], oficial));
        }
      }
    }
    ...
    if (!actividadesEconomicas.isEmpty()) {
      for (String act : actividadesEconomicas) {
        utils.actualizarBase.insertar("territorio_desarrolla_actividad_economica", String.format("('%s', %s)", act.trim().split(" ")[0], territorio_id));
      }
    }
    ...
    for (String gentilicio : gentilicios) {
      if (!gentilicio.isEmpty()) {
        utils.actualizarBase.insertar("gentilicio", String.format("('%s', %s)", gentilicio, territorio_id));
      }
    }
    ...
    if (!subterritorios.isEmpty()) {
      ...
      for (String subterritorio : subterIds) {
        utils.actualizarBase.insertar("territorio_subdivide_en_territorio", String.format("(%s, %s)", territorio_id, subterritorio));
      }
      ...
      if (!vecinos.isEmpty()) {
        ...
        for (String vecino : vcsIds) {
          // x linda con y
          utils.actualizarBase.insertar("territorio_linda_con_territorio", String.format("(%s, %s)", territorio_id, vecino));
          // y linda con x
          utils.actualizarBase.insertar("territorio_linda_con_territorio", String.format("(%s, %s)", vecino.toString(), territorio_id));
        }
      }
